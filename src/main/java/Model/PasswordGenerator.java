...
public class PasswordGenerator {
  ...
  public PasswordGenerator( int minLength,
                            int maxLength ) {
    ...
    List<PasswordCharacterSet> pwSets = new ArrayList<PasswordCharacterSet>(
                               origPwSets.size() );
    for( PasswordCharacterSet origpwSet
         : origPwSets ) {
    ...
    int currentIndex = 0;
    for( PasswordCharacterSet pwSet
         : pwSets ) {
  ...
  public char[] generatePassword() {
    ...
    for( PasswordCharacterSet pwSet
         : pwSets ) {
      addRandomCharacters( pw,
                           pwSet.getCharacters(),
                           pwSet.getMinCharacters(),
                           remainingIndexes,
                           rand );
    }
    addRandomCharacters( pw,
  ...
  private static void addRandomCharacters( char[] pw,
                                           char[] characterSet,
                                           int numCharacters,
                                           List<Integer> remainingIndexes,
                                           Random rand ) {
    for( int i = 0;
         i < numCharacters;
         ++i ) {
      // Get and remove random index from the remaining indexes
      int pwIndex = remainingIndexes.remove( rand.nextInt(
          remainingIndexes.size() ) );
   ...
   */
  private static final class PwSet
    implements PasswordCharacterSet {
    ...
    public PwSet( PasswordCharacterSet pwSet ) {
      ...
      // Defensive copy
      this.chars = Arrays.copyOf( pwSetChars,
                                  pwSetChars.length );
    }
  ...
  private enum SummerCharacterSets
    implements PasswordCharacterSet {
    ALPHA_UPPER( ALPHA_UPPER_CHARACTERS,
                 1 ),
    ALPHA_LOWER( ALPHA_LOWER_CHARACTERS,
                 1 ),
    ...
    private SummerCharacterSets( char[] chars,
                                 int minUsage ) {
      this.chars = chars;
